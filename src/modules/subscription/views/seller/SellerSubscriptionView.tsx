...
export function SellerSubscriptionView() {
  const { stats, subscriptions, loading, createCustomerSubscription, refresh } = useSubscriptionSeller();
  ...
  const handleCreateCustomerSub = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Langganan untuk User #${customerUserId} berhasil diprovisi!`);
    ...
    <div className="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-border/40 pb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Pantau pertumbuhan basis pengguna, komisi reseller, dan provisi paket white-label tenant Anda.
          </p>
        </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-foreground">Daftar Langganan Pelanggan</h2>
          <span className="text-xs text-muted-foreground">Total {subscriptions.length} Akun Terdaftar</span>
        </div>
      ...
      {isProvisionModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
          <div className="w-full max-w-md bg-card border border-border/60 rounded-2xl p-6 shadow-xl space-y-5 animate-in fade-in zoom-in-95 duration-200">
            <h3 className="text-lg font-bold text-foreground">Provisi Langganan Pelanggan</h3>
            <p className="text-xs text-muted-foreground">
              Tentukan ID pelanggan dan paket layanan yang akan diaktifkan secara instan di bawah tenant Anda.
            </p>

            <form onSubmit={handleCreateCustomerSub} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-muted-foreground">Customer User ID</label>
                <input
                  type="number"
                  required
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-muted-foreground">Pilihan Paket</label>
                <select
                  value={planId}
                ...
                >
                  <option value="plan-basic">Basic Tunneling (Rp 25.000 / bln)</option>
                  <option value="plan-pro">Premium Pro Max (Rp 45.000 / bln)</option>
                  <option value="plan-annual">Enterprise Annual Pass (Rp 420.000 / thn)</option>
                </select>
              </div>
