...
export const iamAdminApi = {
  ...
  addBalance: (
    data: AdminAdjustBalanceDto,
    idempotencyKey?: string,
  ): Promise<ApiResponse<UserProfile>> =>
    apiClient.post<UserProfile>("/api/admin/user/add-balance", data, {
      idempotencyKey,
    }),
  ...
  reduceBalance: (
    ...
    apiClient.post<UserProfile>("/api/admin/user/reduce-balance", data, {
      idempotencyKey,
    }),

  // 9. POST /api/admin/user/add-income (Idempotent)
  addIncome: (
    data: AdminAdjustIncomeDto,
    idempotencyKey?: string,
  ): Promise<ApiResponse<UserProfile>> =>
    apiClient.post<UserProfile>("/api/admin/user/add-income", data, {
      idempotencyKey,
    }),
  ...
  reduceIncome: (
    ...
    apiClient.post<UserProfile>("/api/admin/user/reduce-income", data, {
      idempotencyKey,
    }),
