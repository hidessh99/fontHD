...
export function useVpnAdmin() {
  ...
  const triggerHourlyBilling =
    useCallback(async (): Promise<TriggerPayasBillingResponse | null> => {
      setIsBilling(true);
      try {
        const res = await vpnAdminApi.triggerPayasBilling();
        if (res.payload) {
          toast.success(
            `Billing berhasil diproses: Rp ${res.payload.total_billed_amount.toLocaleString()}`,
          );
          return res.payload;
        }
        return null;
        ...
        const msg =
          err instanceof Error ? err.message : "Gagal mengeksekusi billing.";
        toast.error(msg);
        return null;
      } finally {
        setIsBilling(false);
