...
export function SellerQuotaProgress({
  ...
  const percentage = totalQuota > 0 ? Math.round((usedQuota / totalQuota) * 100) : 0;
    ...
    <div className="p-5 rounded-2xl bg-card border border-border/70 space-y-3 font-mono">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-foreground">Kuota Grosir Reseller</span>
        </div>
        <span className="text-xs font-bold text-primary">{percentage}% Terpakai</span>
      </div>

      <Progress value={percentage} className="h-2 rounded-full" />

      <div className="flex items-center justify-between text-xs text-muted-foreground pt-1">
        <span>Sisa: <strong className="text-foreground">{remainingQuota}</strong> Akun</span>
        <span>Total Lisensi: <strong className="text-foreground">{totalQuota}</strong></span>
      </div>
      ...
      {isLow && (
        <div className="flex items-center gap-2 p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs">
          ...
          <span>Sisa kuota grosir Anda menipis. Segera topup untuk melanjutkan penjualan.</span>
        </div>
      )}
    </div>
  );
