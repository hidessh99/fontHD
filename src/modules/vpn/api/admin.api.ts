...
export const vpnAdminApi = {
  ...
  getFreeAccounts: (params?: { page?: number; limit?: number }): Promise<ApiResponse<VpnAccount[]>> =>
    apiClient.get<VpnAccount[]>("/api/vpn/accounts/free", { params }),

  // 2. DELETE /api/vpn/accounts/free/:id
  deleteFreeAccount: (id: number | string): Promise<ApiResponse<{ deleted: boolean }>> =>
  ...
  triggerPayasBilling: (idempotencyKey?: string): Promise<ApiResponse<TriggerPayasBillingResponse>> =>
    apiClient.post<TriggerPayasBillingResponse>("/api/vpn/accounts/payas/billing", {}, { idempotencyKey }),

  // === SERVER CONNECTS (3 Endpoints) ===
  // 4. POST /api/vpn/server-connects
  createServerConnect: (data: CreateServerConnectDto): Promise<ApiResponse<VpnServerConnect>> =>
    apiClient.post<VpnServerConnect>("/api/vpn/server-connects", data),

  // 5. PUT /api/vpn/server-connects/:id
  updateServerConnect: (id: number | string, data: Partial<CreateServerConnectDto>): Promise<ApiResponse<VpnServerConnect>> =>
    apiClient.put<VpnServerConnect>(`/api/vpn/server-connects/${id}`, data),

  // 6. DELETE /api/vpn/server-connects/:id
  deleteServerConnect: (id: number | string): Promise<ApiResponse<{ deleted: boolean }>> =>
    apiClient.delete<{ deleted: boolean }>(`/api/vpn/server-connects/${id}`),

  // === SERVER TYPES (3 Endpoints) ===
  // 7. POST /api/vpn/server-types
  createServerType: (data: CreateServerTypeDto): Promise<ApiResponse<VpnServerType>> =>
    apiClient.post<VpnServerType>("/api/vpn/server-types", data),

  // 8. PUT /api/vpn/server-types/:key_name
  updateServerType: (keyName: string, data: Partial<CreateServerTypeDto>): Promise<ApiResponse<VpnServerType>> =>
    apiClient.put<VpnServerType>(`/api/vpn/server-types/${keyName}`, data),

  // 9. DELETE /api/vpn/server-types/:key_name
  deleteServerType: (keyName: string): Promise<ApiResponse<{ deleted: boolean }>> =>
